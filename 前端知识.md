...
3. 不要使用纯样式标签，如：b、font、u等，改用 css 设置

4. 需要强调的文本，可以包含在 strong 或者 em 标签中（浏览器预设样式，能用 CSS 指定就不用他们），strong 默认样式是加粗（不要用b），em 是斜体（不用i）
...
offsetTop——距离有定位的祖先元素顶部距离

offsetLeft——距离有定位的祖先元素左部距离
##### 居中方法
水平方向上
```css
...
```
![三角形](https://ph-image-test.oss-ap-southeast-1.aliyuncs.com/LyHEX5r.1tj.png)

**原理:**
以一原点,上下左右距离形成矩形,再以原点到各个边填充不同颜色形成三角形
